...
parser.add_argument('--testMicrophone', type=int, default=0, help='Flag for saving microphone audio in a wavfile, to check if it is capturing well [0  for false, 1 for true]')
args = parser.parse_args()
print("=> Iniciando com parâmetros:")
print(f'--modelPath: {args.modelPath}')
print(f'--duration: {args.duration}')
print(f'--wavPath: {args.wavPath}')
print(f'--testMicrophone: {args.testMicrophone}')
...
def classify(y, sr):
    print(f"=> Extraindo features com {extract_method}")
    ...
    predicted_class = encoded_labels[np.argmax(predictions)]
...
def record_and_classify():
    ...
    if args.wavPath != '':
        ...
        audio_data = sd.rec(int(args.duration * sd.default.samplerate), channels=1, dtype='int16')
        sd.wait()
        # Convert the audio data to a format compatible with librosa.load
        audio_data_float = audio_data.flatten().astype(np.float32) / np.iinfo(np.int16).max
        sample_rate = sd.default.samplerate
    
        if args.testMicrophone == 1:
            test_microphone(audio_data, sd.default.samplerate) 
    
    print("Audio capturado...Iniciando processamento")

    predicted_class = classify(audio_data_float, sample_rate)
    print("Processamento finalizado.")
    print(f"A emoção inferida é: {predicted_class}")
